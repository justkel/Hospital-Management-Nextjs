          ...
          <div className="flex-1">
            ...
            {trend && trendValue && (
              <div className="mt-2 flex items-center gap-1.5">
                ...
                <span
                  className={`text-[12px] font-medium ${trend === 'up'
                    ? 'text-emerald-600'
                    : trend === 'down'
                      ? 'text-rose-600'
                      : 'text-gray-500'
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <MetricCard
            ...
            trend={overview.visits.openVisits > 5 ? 'up' : 'down'}
            trendValue={
              overview.visits.openVisits > 5 ? '+3 from yesterday' : '-2 from yesterday'
            }
            color="purple"
          />
          <MetricCard
          ...
          {overview.laboratory && (
            <MetricCard
              label="Pending Lab"
              value={overview.laboratory.pending}
              icon={<Syringe className="h-5 w-5" />}
              color="orange"
              subtitle={`${overview.laboratory.inProgress} in progress, ${overview.laboratory.urgent} urgent`}
            />
          )}
          {overview.beds && (
            <MetricCard
              label="Bed Occupancy"
              value={`${overview.beds.occupied}/${overview.beds.totalActive}`}
              icon={<Bed className="h-5 w-5" />}
              color="teal"
              trend={
                overview.beds.occupied / overview.beds.totalActive > 0.8
                  ? 'up'
                  : 'down'
              }
              trendValue={`${Math.round((overview.beds.occupied / overview.beds.totalActive) * 100)}% occupied`}
            />
          )}
          {overview.theatre && (
            <MetricCard
              label="Today's Procedures"
              value={overview.theatre.todayProcedures}
          ...
          {overview.incidents && (
            <MetricCard
              ...
              value={overview.incidents.activeTotal}
              icon={<AlertTriangle className="h-5 w-5" />}
              color={overview.incidents.activeTotal > 0 ? 'red' : 'green'}
              trend={
                overview.incidents.activeTotal > 0 ? 'up' : 'neutral'
              }
              trendValue={
                overview.incidents.activeTotal > 0
                  ? 'Requires attention'
                  : 'All clear'
              }
            />
          ...
          {overview.financial && (
            <MetricCard
              label="Revenue Today"
              value={money(overview.financial.revenueToday)}
              icon={<CreditCard className="h-5 w-5" />}
              color="green"
              subtitle={`${overview.financial.paymentsReceivedInPeriod} payments`}
            />
          )}
        </div>
        ...
        {(overview.laboratory || overview.theatre || overview.financial) && (
          <div className="grid gap-6 lg:grid-cols-3">
              ...
              >
                <div className="grid grid-cols-3 gap-3">
                  {[
                    {
                      label: 'Urgent',
                      value: overview.laboratory.urgent,
                      color: 'bg-rose-100 text-rose-700',
                    },
                    {
                      label: 'In Progress',
                      value: overview.laboratory.inProgress,
                      color: 'bg-blue-100 text-blue-700',
                    },
                    {
                      label: 'Completed',
                      value: overview.laboratory.completedInPeriod,
            ...
            {overview.theatre && (
              ...
              >
                <div className="space-y-2">
                  {overview.theatre.upcomingSchedule.length ? (
                    overview.theatre.upcomingSchedule
                      .slice(0, 4)
                      .map((item) => (
                        <div
                          key={item.id}
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between rounded-xl bg-emerald-50 px-4 py-3">
                    ...
                    <span className="text-[16px] font-bold text-emerald-800">
                      {money(overview.financial.revenueInPeriod)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-amber-50 px-4 py-3">
                    ...
                    <span className="text-[16px] font-bold text-amber-800">
                      {money(overview.financial.overallOutstandingBalance)}
                    </span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    <span className="text-[16px] font-bold text-gray-800">
                      {overview.financial.paymentsReceivedInPeriod}
                    </span>
                  </div>
                </div>
              </Section>
